...
# class RayNetworkManager(object):
#   def __init__(self, ncardtypes, nbets, nactions, embed_dim, device=torch.device("cpu")):
#     print("Initialized RayNetworkManger")
#     self._net1 = DeepCFRModel(ncardtypes, nbets, nactions, embed_dim).to(device)
#     self._net2 = DeepCFRModel(ncardtypes, nbets, nactions, embed_dim).to(device)
...
#   def get_action_probabilities(self, player_uuid, infoset):
#     """
#     Takes an infoset, passes it into the network, and returns the action probabilities predicted
#     by the network.
#     """
...
#       if player_uuid == Constants.PLAYER1_UID:
#         normalized_adv = self._net1(cards_input, bets_input)[0]
#       else:
#         normalized_adv = self._net2(cards_input, bets_input)[0]
