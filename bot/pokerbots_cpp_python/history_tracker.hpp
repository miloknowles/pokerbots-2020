...
class HistoryTracker {
 public:
  ...
  cfr::FlexHistory History() const { return history_; }
 ...
 private:
  ...
  cfr::FlexHistory history_{};
};

inline cfr::EvInfoSet MakeInfoSet(const HistoryTracker& ht, int player_idx, bool player_is_sb, float ev, int current_street) {
  cfr::FixedHistory fh;
  std::fill(fh.begin(), fh.end(), 0);

  cfr::FlexHistory history = ht.History();
  for (int street = 0; street < history.size(); ++street) {
    const std::vector<int>& actions_this_street = history.at(street);
    const int offset = street * cfr::kMaxActionsPerStreet + (street > 0 ? 2 : 0);

    for (int i = 0; i < actions_this_street.size(); ++i) {
      const int max_this_street = (street > 0) ? cfr::kMaxActionsPerStreet : (cfr::kMaxActionsPerStreet + 2);
      const int wrap = std::min(i, max_this_street - 2 + i % 2);
      fh.at(offset + wrap) += actions_this_street.at(i);
    }
  }

  return cfr::EvInfoSet(ev, fh, player_is_sb ? 0 : 1, cfr::GetStreet0123(current_street));
}
