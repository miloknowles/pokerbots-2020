...
from network_wrapper import NetworkWrapper, RayNetworkWrapper, RayNetworkManager
from trainer import generate_actions, make_infoset


NUM_TRAVERSALS_TOTAL = 1000
NUM_PROCESSES = 12
...
if __name__ == '__main__':
  ...
  traverse_player = Constants.PLAYER1_UID
  
  t0 = time.time()

  try:
    results = ray.get([traverse_multiple.remote(i, traverse_player, strategies, advantage_mem, strategy_mem, t) for i in range(NUM_PROCESSES)])
  except KeyboardInterrupt:
    ray.shutdown()
    exit(0)
